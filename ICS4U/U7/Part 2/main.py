...
def qb_information(file): #finds qb information: first name, last name, and passer rate
    for line in file.readlines(): #reads every line in "qbdata.txt"
...
def qb_name(file): #lists qb last name and first name
    for line in file.readlines(): #reads every line in "qbdata.txt"
        values = line.split() #gets the different values, creates a list
        first_name = values[0] #gets first name
        last_name = values[1] #gets last name
...
def highest_rated_player(file): #finds highest rated player 
...
def average_num_of_yards(file): #finds average number of yards completed per pass
    for line in file.readlines(): #reads every line in "qbdata.txt"
...
def over_three_hundred_passes(file): #finds # of players with over 300 completed passes 
    count = 0 #counter 
    for line in file.readlines(): #reads every line in "qbdata.txt"
            values = [] #variable for values
            values = line.split() #gets the different values, creates a list
...
def main():
    try:
        while True: #while user wants to run program
            file = open(r'ICS4U/U7/Part 2/qbdata.txt','r') #opens file "qbdata.txt"; includes path
